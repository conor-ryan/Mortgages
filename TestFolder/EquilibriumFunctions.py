...
def solve_eq(alpha,d,theta,m,**kwargs):
    ...
    dump, grad_k, foc_k = Derivatives.d_foc(r,alpha,d,theta,m)
...
# m - MBS pricing function 
# kwargs (r_start) - allow for an optional starting vector
def solve_eq_robust(alpha,d,theta,m,**kwargs):
    # Define zero profit interest rate which will help bound feasible equi. interest rates
    r_min = ModelFunctions.min_rate(d,theta,m) 
    # Check if marginal cost pricing is an equilibrium 
    # Occurs if the probability of purchase is very small
    ...
    foc_k = ModelFunctions.expected_foc_nonlinear(r,alpha,d,theta,m)
    ...
    while err>1e-12 and itr<1000:
        ...
        foc_next = ModelFunctions.expected_foc_nonlinear(r_new,alpha,d,theta,m)
...
# m - MBS pricing function 
# kwargs (r_start) - allow for an optional starting vector
def solve_eq_very_robust(alpha,d,theta,m,**kwargs):
    # Define zero profit interest rate which will help bound feasible equi. interest rates
    r_min = ModelFunctions.min_rate(d,theta,m) 
...
## Hold Only - Equilibrium Solution Given Consumer Price Elasticity Parameter
## Used for counterfactual without OTD
# alpha - consumer specific price elasticity parameter
# d - data object
# theta - parameter object
# m - MBS pricing function

def solve_eq_hold_only(alpha,d,theta,m):
    # Define Initial Price Vector as average markup over zero-profit HTM rate   
    r_min = ModelFunctions.r_hold_min(theta,d) 
    r = r_min + 1 /(-alpha*(1 - 1/len(r_min)))
    
    ## Algorithm follows quasi-newton method
    # Set initial step size
    B_k_inv = np.repeat(1e-4,len(r))
    # Initial function evaluation
    foc_k = ModelFunctions.hold_only_foc(r,alpha,d,theta,m)
    # Initialize error and iteration number
    err = 1 
    itr = 0
    # Iterate until a tolerance of 1e-12
    while err>1e-12:
        itr = itr+1
        # Newtown step for new r vector
        r_new = r + B_k_inv*foc_k
        # Bound the r vector below
        r_new = np.maximum(r_new,-0.01)
        # Bound the r vector above
        # r_new = np.minimum(r_new,r_min*15)
        # Compute the (bounded) step size
        step = r_new - r
        # Update r vector
        r = r_new 
        ...
        # Evaluate Function 
        foc_next =  ModelFunctions.hold_only_foc(r,alpha,d,theta,m)
        # Compute change in function value for step update
        y_k = foc_next - foc_k
        # Update step size
        B_k_inv = np.abs((step/y_k))
        # Update step size for bounded values
        B_k_inv[step==0] = 1e-4
...
# theta - parameter object
# m - MBS pricing function
def solve_eq_r(r0,j,d,theta,m,itr_max=100):
    # Define zero profit interest rate which will help bound feasible equi. interest rates
    r_min = ModelFunctions.min_rate(d,theta,m) 
    ...
    r, itr = solve_eq(alpha,d,theta,m)
    ...
    grad_alpha, grad_k, foc_k =  Derivatives.d_foc(r,alpha,d,theta,m)
...
# theta - parameter object
# m - MBS pricing function
def solve_eq_r_robust(r0,j,d,theta,m):
    ...
    if alpha>(-800):
        ...
        r, itr = solve_eq_robust(alpha,d,theta,m)
    else: 
        r, itr = solve_eq_very_robust(alpha,d,theta,m)
    ...
    while (err>1e-12) & (itr<12000):
        ...
        # Apply increasingly robust solution methods for greater values of alpha
        if alpha>(-900):
            r, i_new = solve_eq(alpha,d,theta,m)
        elif alpha>(-10000):
            r, i_new = solve_eq_robust(alpha,d,theta,m)
        else: 
            r, i_new = solve_eq_very_robust(alpha,d,theta,m)
...
def solve_r_cons_mkp(r0,j,d,theta,m):
    prof = ModelFunctions.expected_sale_profit(r0,d,theta,m)[j]
    if prof<0:
        alpha = -1000
        r = np.repeat(r0,d.X.shape[0])
        return alpha, r, -1
    
    def obj_fun(x):
        return ModelFunctions.target_markup(x,prof,d,theta,m)
    
    res = sp.optimize.root(obj_fun,np.repeat(r0,d.X.shape[0]))
    alpha = -1/prof
    return alpha,res.x,res.nfev

def solve_eq_optim(alpha,d,theta,m,**kwargs):
    # Define zero profit interest rate which will help bound feasible equi. interest rates
    r_min = ModelFunctions.min_rate(d,theta,m) 
    ...
    def obj_fun(x):
        dump, grad_k, foc_k = Derivatives.d_foc(x,alpha,d,theta,m)
        return foc_k, grad_k
    
    res = sp.optimize.root(obj_fun,r,jac=True)

    return res.x, res.fev


def solve_eq_r_optim(r0,j,d,theta,m):
    # Try Fast Method
    itr_max = 50
    alpha, r, itr = solve_eq_r(r0,j,d,theta,m,itr_max=itr_max)
    if itr<itr_max:
    ...
    dpi = ModelFunctions.expected_sale_profit(r0,d,theta,m)[j]
    ...
    def obj_fun(x):
        ...
        g_alpha, grad, f = Derivatives.d_foc(r,alpha,d,theta,m)
        grad[j,:] = g_alpha
