...
def d_foc(r,alpha,d,theta,m):
    ...
    tol =1e-6
    if any(q==1) or any(q==0):
        q = q*(1-tol) + tol*(1/len(q))
    ...
    ## Second derivative of MBS Price (Probably 0 in implementation)
    d2Pdr2 = m.d2Pdr2(r)
    ...
    dFOC_dalpha = ( -(prob_h*dpi_h+prob_s*dpi_s)/(alpha*(1-q))**2 ) * ( 1-q-alpha*dqdalpha )
    ...
    infra_mat = np.tile((prob_h*dpi_h+prob_s*dpi_s),(len(q),1))
    dFOC_dr = -(infra_mat/(1-q_mat)**2)*(q_mat*q_mat_t)

    diag_vals = (prob_h*dpi_h+prob_s*dpi_s)*q/(1-q) + prob_h*dpi_h + prob_s*dpi_s + prob_s*d2Pdr2/(alpha*(1-q))
    diag_vals = diag_vals + dProb_h_dr*(dpi_h/(alpha*(1-q)) + pi_h) + dProb_s_dr*(dpi_s/(alpha*(1-q)) + pi_s)
...
def d_foc_all_parameters(r,alpha,d,theta,m):
    ...
    # Matrix of infra-marginal return
    ...
    infra_mat = np.tile((prob_h*dpi_h+prob_s*dpi_s),(dqdbeta_x.shape[0],1))
    dFOC_dbeta_x = (infra_mat/(alpha*(1-q_mat_x)**2))*(dqdbeta_x)

    ## Derivative of FOC w.r.t. gamma - Columns: FOC_j, Rows: gamma_k
    dFOC_dgamma = -np.transpose(np.concatenate((d.W,np.tile(d.Z,(J,1))),axis=1))
...
def d2_foc_all_parameters(r,alpha,d,theta,m):
    ...
    ## Higher-order derivatives of MBS Price (Probably 0 in implementation)
    d2Pdr2 = m.d2Pdr2(r)
    d3Pdr3 = m.d3Pdr3(r)
    ...
    d2Prob_h_dr2 = ((dpi_h - dpi_s)*(prob_h*dProb_s_dr + prob_s*dProb_h_dr)-d2Pdr2*prob_h*prob_s)/theta.sigma
    d2Prob_s_dr2 = -d2Prob_h_dr2


    ## Derivative of FOC w.r.t. Alpha
    dFOC_dalpha = ( -(prob_h*dpi_h+prob_s*dpi_s)/(alpha*(1-q))**2 ) * ( 1-q-alpha*dqdalpha )
    d2FOC_dalpha2 = -(prob_h*dpi_h+prob_s*dpi_s)*( (1/(alpha*(1-q))**2 )*( -dqdalpha - (dqdalpha + alpha*d2qdalpha2) ) + 
    ...
    infra = prob_h*dpi_h+prob_s*dpi_s
    for j in range(J):
    ...
    diag_vals = ((prob_h*dpi_h+prob_s*dpi_s)*(dqdalpha/(1-q) + q*dqdalpha/(1-q)**2) - 
                    (1/(alpha*(1-q) )**2)*(1-q-alpha*dqdalpha)*(dProb_h_dr*dpi_h +dProb_s_dr*dpi_s + prob_s*d2Pdr2 ) )
    ...
    for j in range(J):
        ...
        j_sym = infra[j]*(-alpha*q[j]*q/(1-q[j])-alpha*q[j]**2*q/(1-q[j])**2) - \
                            (q[j]*q/(1-q[j])**2)*(dProb_h_dr[j]*dpi_h +dProb_s_dr[j]*dpi_s[j] + prob_s[j]*d2Pdr2[j])
        

        d2FOC_dr2[j,j,:] = j_sym
        d2FOC_dr2[j,:,j] = j_sym
        d2FOC_dr2[j,j,j] = infra[j]*(alpha*q[j]+alpha*q[j]**2/(1-q[j]) ) +\
                            2*q[j]/(1-q[j])*(dProb_h_dr[j]*dpi_h +dProb_s_dr[j]*dpi_s[j] + prob_s[j]*d2Pdr2[j]) +\
                            d2Prob_h_dr2[j]*(dpi_h/(alpha*(1-q[j])) + pi_h[j]) +\
                            d2Prob_s_dr2[j]*(dpi_s[j]/(alpha*(1-q[j])) + pi_s[j]) +\
                            dProb_h_dr[j]*dpi_h +dProb_s_dr[j]*dpi_s[j] + prob_s[j]*d2Pdr2[j] +\
                            2*dProb_s_dr[j]*d2Pdr2[j]/(alpha*(1-q[j])) +\
                            prob_s[j]*(d3Pdr3[j]/(alpha*(1-q[j])))
    ...
    for j in range(J):
        ...
        d2FOC_drdbeta[j,j,:] = infra[j]*( dqdbeta_x[:,j]/(1-q[j])*(1 + q[j]/(1-q[j])) ) +\
                                dqdbeta_x[:,j]/(alpha*(1-q[j])**2)*(dProb_h_dr[j]*dpi_h + dProb_s_dr[j]*dpi_s[j] + prob_s[j]*d2Pdr2[j])
...
def share_parameter_second_derivatives(r,alpha,d,theta,m):
    ## Evaluate all FOC derivatives
    df_dendo,df_dtheta, d2f_dendo2, d2f_dendodtheta, d2f_dtheta2 = d2_foc_all_parameters(r,alpha,d,theta,m)
...
def share_parameter_derivatives(r,alpha,d,theta,m):
    ## Evaluate all FOC derivatives
    dfdalph, df_dr, df_db, df_dg = d_foc_all_parameters(r,alpha,d,theta,m)
