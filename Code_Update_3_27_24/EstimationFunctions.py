...
def consumer_likelihood_eval(theta,d,m):
    ...
    alpha, r_eq, itr = EquilibriumFunctions.solve_eq_r(d.r_obs,d.lender_obs,d,theta,m)
    print("Solved",d.i,itr)
    if itr==100: # Fast algorithm failed to converge, use robust algorithm
...
def evaluate_likelihood_gradient(x,theta,cdf,mdf,mbsdf):
    ...
    for i in range(cdf.shape[0]):
        ...
        dw_i   = dw_i.astype(float) 
        dll_i  = dll_i.astype(float) 
...
def evaluate_likelihood_hessian(x,theta,cdf,mdf,mbsdf):
    ...
    for i in range(cdf.shape[0]):
        ...
        dw_i   = dw_i.astype(float)
        d2w_i  = d2w_i.astype(float) 
        dll_i  = dll_i.astype(float) 
        d2ll_i = d2ll_i.astype(float) 
...
def estimate_NR(x,theta,cdf,mdf,mbsdf):
    ...
    test_index = range(0,len(theta.beta_x)) #range(0,3)
    ...
    while err>1e-6:
        ...
        while np.isnan(ll_new) or ll_new<ll_k*1.0005 : # Allow a small step in the wrong direction
...
def estimate_GA(x,theta,cdf,mdf,mbsdf):
    # Testing Tool: A index of parameters to estimate while holding others constant
    # This can help identification. range(0,len(x)) will estimate all parameters 
    test_index = range(0,len(theta.beta_x))
    # Set candidate vector in parameter object
    theta.set(x)
