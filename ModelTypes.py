...
class Data:
    def __init__(self,i,X,W,Z,lender,r,out):
...
class Parameters:
    ...
    def __init__(self,
                 demand_spec,cost_spec, # Model Specifications
                 rate_spec,lender_spec,market_spec,out_spec, # Model Specifications
                 cost_diff,sigma,psi,min_rate,# Parameters from First Stage estimation 
        ...
        self.gamma_WS = np.zeros(len(cost_spec)) # Initialize HTM Cost parameters

        ### Transfer First Stage Parameters ### 
        self.gamma_diff = cost_diff
        self.psi = psi
        self.sigma = sigma
    ...
    def all(self):
        return np.concatenate( (self.beta_x,self.gamma_WH ))
...
class Parameters_Sim:
    def __init__(self,β_x,γ_WH,γ_WS,σ,psi):
        self.beta_x = β_x
        self.gamma_WH = γ_WH
        self.gamma_WS = γ_WS
        self.gamma_diff = γ_WS-γ_WH
        self.psi = psi
        self.sigma = σ