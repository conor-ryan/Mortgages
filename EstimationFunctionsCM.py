...
def consumer_likelihood_eval(theta,d,m):
    ...
    alpha, r_eq, itr = EquilibriumFunctions.solve_r_cons_mkp(d.r_obs,d.lender_obs,d,theta,m)
...
def consumer_likelihood_eval_gradient(theta,d,m):
    ...
    # Potential Stability problem: one market share is close to 100% 
        ...
        dlogq, dq0 = Derivatives_CnstMkup.cnst_mkup_derivatives(r_eq,alpha,d,theta,m)
...
def consumer_likelihood_eval_hessian(theta,d,m):
    ...
    # Potential Stability problem: one market share is close to 100% 
        ...
        dlogq, d2logq, dq0,d2q0  = Derivatives_CnstMkup.cnst_mkup_second_derivatives(r_eq,alpha,d,theta,m)
        # Compute likelihood and probability weight gradients
        dll_i = dlogq[:,d.lender_obs] 
        dw = dq0/(1-q0)**2
...
def evaluate_likelihood_gradient(x,theta,cdf,mdf,mbsdf):
    ...
    pred_N = np.zeros(len(theta.out_share))
    dpred_N = np.zeros((len(theta.out_share),len(theta.all())))
    ...
    if any(pred_N==0):
        return -1e7, np.repeat(0,len(theta.all()))
...
def evaluate_likelihood_hessian(x,theta,cdf,mdf,mbsdf):
    ...
    for i in range(cdf.shape[0]):
        ...
        itr_avg += max(itr,0)
    if any(pred_N==0):
        return -1e7, np.repeat(0,len(theta.all())), np.tile(0,(len(theta.all()),len(theta.all())))
...
def estimate_NR(x,theta,cdf,mdf,mbsdf):
    ...
    while err>1e-6:
        ...
        itr+=1 
        print("#### Iteration",itr, "Error", err, "Value", ll_k)
...
def py_optim(x,theta,cdf,mdf,mbsdf):

    def f_obj(x):
        val, g = evaluate_likelihood_gradient(x,theta,cdf,mdf,mbsdf)
        err = np.sqrt(np.dot(g,g))
        print("Gradient Size:", err)
        return -val, -g
    
    def f_hess(x):
        ll, g,h = evaluate_likelihood_hessian(x,theta,cdf,mdf,mbsdf)
        return -h
...
def py_optim2(x,theta,cdf,mdf,mbsdf):
    ...
    res = sp.optimize.minimize(f_obj,x,method="trust-ncg",jac=True,hess=f_hess,
                               options={'max_trust_radius':5,'initial_trust_radius':1})
    return res
